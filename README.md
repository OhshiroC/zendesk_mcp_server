...
| `get_ticket` | チケット詳細 + カスタムフィールド + コメント一覧を取得 |
...
クライアントに返す応答から個人情報（ユーザー名・メールアドレス・電話番号）を
自動でマスクします。**デフォルトで有効**です。

- `get_user` の名前・メールは `[NAME]` / `[EMAIL]` に置換されます。
- `get_ticket` のカスタムフィールドのうち、氏名を保持するもの（「名前」「氏名」
  「お名前」）の値は `[NAME]` に置換されます。
- コメント本文・CSAT コメント・チケット件名などの自由記述中のメールアドレス・
  電話番号は `[EMAIL]` / `[PHONE]` に置換されます。
...
> 誤マスク等の問題が発生した場合は `ZENDESK_MASK_PII=0` を設定してサーバーを
> 再起動することで、即座にマスクを無効化できます。
...
2. インストール時に表示される設定画面で、以下を入力する:
   ...
   - **PII マスキング**（任意）: 個人情報（ユーザー名・メールアドレス・電話番号）のマスクを有効/無効にします。**デフォルトは有効**。オフにすると生の値がそのまま返ります。
